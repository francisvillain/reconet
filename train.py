...
def imsave(tensor, title=None):
    ...
    image.save("udnie/test_img_{}.jpg".format(title))
    #plt.imshow(image)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--style",default="./styles/udnie.jpg", help="Path to style image")
    parser.add_argument("--data-dir", default="F:/dataset", help="Path to data root dir")
    ...
    parser.add_argument("--output-file", default="./model_min_udnie.pth", help="Output model file path")
        ...
        for epoch in range(n_epochs):
            for sample in traindata:
                ...
                with torch.no_grad():
                    ...
                    if global_step % 500 == 0 or last_iteration:
                    ...
                    if global_step % 1000 == 0 or last_iteration:
                        print(time.ctime())
                        print()
                        print(losses["training loss"])
                        styled_test_image = stylize_image(Image.open("test_image.jpg"), model)
                        #writer.add_image('test image', styled_test_image, global_step)
                        imsave(styled_test_image,str(global_step))
                        #styled_test_image.save('test_image_{}.jpg'.format(global_step))
                    ...
                    if global_step % 5000 == 0:
                        torch.save(model.state_dict(),'./udnie/model_min_{}.pth'.format(global_step))
                    global_step += 1
